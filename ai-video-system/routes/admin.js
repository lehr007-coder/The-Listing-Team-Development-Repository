...
async function jobDiagnose(env, jobId) {
  ...
  if (job.status === "delivered") {
    ...
    if (ageMin > 15) {
      parts.push(`HeyGen still processing after ${ageMin} min — unusually slow. Likely cause: credits exhausted mid-render.`);
    } else {
...
async function jobArchive(env, jobId) {
  ...
  try {
    ...
    if (!r.ok) throw new Error(`source fetch ${r.status} ${job.r2_url}`);
    ...
    await updateVideoJob(env, jobId, { r2_key: r2Key, r2_url: newR2Url });
    return json({
      ok: true, job_id: jobId, r2_key: r2Key, r2_url: newR2Url,
      bytes: buf.byteLength, fetch_ms: Date.now() - fetchStart - putMs, put_ms: putMs,
    });
  } catch (e) {
...
async function jobTracking(env, jobId) {
  ...
  const watch = { 25: counts["watch_25"] || 0, 50: counts["watch_50"] || 0, 75: counts["watch_75"] || 0, 100: counts["watch_100"] || 0 };
  const max_watch_pct = watch[100] ? 100 : watch[75] ? 75 : watch[50] ? 50 : watch[25] ? 25 : 0;
  ...
  return json({
    job_id: jobId, contact_id: job.contact_id, video_type: job.video_type,
    render_engine: job.render_engine, status: job.status, hosted_url: job.hosted_url,
    delivered_at: job.delivered_at, counts,
    summary: {
      opens: counts["open"] || 0, clicks: counts["click"] || 0, cta_clicks: counts["cta_click"] || 0,
      max_watch_pct, watch_milestones: watch,
      sent: { email: counts["sent_email"] || 0, sms: counts["sent_sms"] || 0, conversation: counts["sent_conversation"] || 0 },
      ghl_notes_appended: counts["ghl_note_appended"] || 0, engagement_score,
      total_events: events.length, first_event_at: first, last_event_at: last,
    },
    timeline: events,
  });
}
...
async function dailySummary(env, url) {
  ...
  const [jobsR, eventsR] = await Promise.all([
    fetch(`${env.SUPABASE_URL}/rest/v1/video_jobs?created_at=gte.${encodeURIComponent(since)}&select=id,status,render_engine,delivery_channels,engagement_score&limit=2000`, { headers: sbHeaders(env) }),
    fetch(`${env.SUPABASE_URL}/rest/v1/video_events?created_at=gte.${encodeURIComponent(since)}&select=event,meta&limit=5000`, { headers: sbHeaders(env) }),
  ]);
  ...
  const eventCounts = {}, opensBySrc = {}, clicksBySrc = {};
  ...
  for (const ch of ["email", "sms", "conversation"]) {
    ...
    ctr[ch] = { sent, clicks, ctr_pct: sent > 0 ? +(clicks / sent * 100).toFixed(1) : null };
  }

  const w25 = eventCounts.watch_25 || 0, w50 = eventCounts.watch_50 || 0;
  const w75 = eventCounts.watch_75 || 0, w100 = eventCounts.watch_100 || 0;

  return json({
    window: { days, since, until: new Date().toISOString() },
    jobs: { total: jobs.length, by_status: byStatus, by_engine: byEngine, total_engagement: totalEngagement },
    events: { total: events.length, counts: eventCounts },
    ctr_by_channel: ctr, opens_by_src: opensBySrc, clicks_by_src: clicksBySrc,
    watch_funnel: { "25": w25, "50": w50, "75": w75, "100": w100 },
    delivery_rate_pct: jobs.length > 0 ? +((byStatus.delivered || 0) / jobs.length * 100).toFixed(1) : null,
  });
}
...
async function weeklyReportSend(env, request) {
  ...
  try {
    ...
    return json({ ok: false, reason: "internal_error", error: e.message, stack: (e.stack || "").split("\n").slice(0, 5).join(" | ") }, 500);
  }
}
...
async function contactsSyncScores(env, request, url) {
  ...
  if (dryRun) {
    return json({ ok: true, dry_run: true, total_contacts_with_videos, contacts_in_batch: batch.length, truncated: total_contacts_with_videos > max, preview: batch });
  }
  ...
  return json({
    ...
    total_contacts_with_videos, contacts_attempted: batch.length,
    contacts_synced: synced, contacts_failed: batch.length - synced,
    truncated: total_contacts_with_videos > max, results,
  });
}
...
async function contactLookup(env, url) {
  ...
  try {
    ...
    return json({ ok: true, email, found: true, contact_id: c.id, first_name: c.firstName || null, last_name: c.lastName || null, location_id: c.locationId || null });
  } catch (e) {
...
async function topContacts(env, url) {
  ...
  const r = await fetch(
    `${env.SUPABASE_URL}/rest/v1/video_jobs?contact_id=not.is.null&select=contact_id,status,engagement_score,created_at&order=created_at.desc&limit=2000`,
    { headers: sbHeaders(env) }
  );
  ...
  for (const j of jobs) {
    ...
    const c = byContact[j.contact_id] = byContact[j.contact_id] || { contact_id: j.contact_id, total_videos: 0, delivered: 0, failed: 0, total_engagement: 0, last_render_at: null };
    c.total_videos++;
    if (j.status === "delivered") c.delivered++;
    if (j.status === "failed")    c.failed++;
    c.total_engagement += (j.engagement_score || 0);
    if (!c.last_render_at || j.created_at > c.last_render_at) c.last_render_at = j.created_at;
  }

  const top = Object.values(byContact).sort((a, b) => (b.total_engagement - a.total_engagement) || (b.total_videos - a.total_videos)).slice(0, limit);
  return json({ count: top.length, contacts: top });
}

async function contactVideos(env, contactId) {
  const r = await fetch(
    `${env.SUPABASE_URL}/rest/v1/video_jobs?contact_id=eq.${encodeURIComponent(contactId)}&order=created_at.desc&limit=100`,
    { headers: sbHeaders(env) }
  );
  if (!r.ok) return error(502, "supabase_error", await r.text());
  const jobs = await r.json();
  return json({
    contact_id: contactId, count: jobs.length,
    videos: jobs.map(j => ({ job_id: j.id, video_type: j.video_type, render_engine: j.render_engine, status: j.status, hosted_url: j.hosted_url, created_at: j.created_at, delivered_at: j.delivered_at, engagement_score: j.engagement_score })),
  });
}
...
async function jobFail(env, jobId, request) {
  ...
  const updated = await updateVideoJob(env, jobId, { status: "failed", failed_at: nowIso(), error: reason });
...
async function killSet(request, env) {
  const { body } = await readJson(request);
  if (body?.action === "clear" || body?.kill === false) return killClear(env);
  const result = await setKillSwitch(env, true, { reason: body?.reason || "no reason provided", set_by: body?.set_by || "admin-api" });
...
const AGENT_SAMPLES = {
  heygen_script: {
    contact: { first_name: "Maria", last_name: "Hernandez", email: "maria@example.com", phone: "+15551234567" },
    lead_intelligence: { lead_score: 78, ylopo_last_event: "viewed_listing_3x", property_interest: "single_family_$450k_$600k", neighborhood: "Coral Gables, FL", days_in_pipeline: 12 },
    listing: { address: "123 Sunset Blvd, Coral Gables, FL", list_price: 575000, beds: 4, baths: 3, sqft: 2400 },
    agent_first_name: "Scott", agent_brand: "The Listing Team", video_type: "new_listing_match",
  },
  fcpxml_director: {
    listing: { address: "123 Sunset Blvd, Coral Gables, FL", list_price: 575000, beds: 4, baths: 3, sqft: 2400, photos: ["https://example.com/photo1.jpg", "https://example.com/photo2.jpg", "https://example.com/photo3.jpg"] },
    agent_brand: "The Listing Team", target_platforms: ["tiktok", "instagram_reels", "youtube_shorts"],
  },
  video_delivery: {
    job_id: "vj_test_sample", video_type: "new_listing_match",
    ...
    agent_first_name: "Scott", agent_brand: "The Listing Team",
    script: "Hi Maria, I just found a beautiful 4-bed in Coral Gables...",
  },
  social_content: {
    listing: { address: "123 Sunset Blvd, Coral Gables, FL", list_price: 575000, beds: 4, baths: 3 },
    video_url: "https://videos.reallistingteam.com/v/vj_test_sample", agent_brand: "The Listing Team",
  },
};
...
async function agentsTest(env, request) {
  ...
  if (!agent || !AGENT_SAMPLES[agent]) {
    return error(400, "invalid_agent", `agent must be one of: ${Object.keys(AGENT_SAMPLES).join(", ")}`);
  }
  ...
  try {
    ...
    return json({ ok: true, agent, provider: env[agentEndpointVar(agent)] ? "agent_studio" : (env.AGENT_FALLBACK_PROVIDER || "anthropic"), latency_ms, context_used: context, output });
  } catch (e) {
    return json({ ok: false, agent, error: e.message, context_used: context, latency_ms: Date.now() - startedAt }, 502);
  }
}

async function cfImagesTest(env) {
  if (!env.CF_ACCOUNT_ID) return json({ ok: false, reason: "CF_ACCOUNT_ID not set" }, 503);
  if (!env.CF_IMAGES_API_TOKEN) return json({ ok: false, reason: "CF_IMAGES_API_TOKEN not set" }, 503);
  if (!env.CF_IMAGES_ACCOUNT_HASH) return json({ ok: false, reason: "CF_IMAGES_ACCOUNT_HASH not set" }, 503);
  const r = await fetch(`https://api.cloudflare.com/client/v4/accounts/${env.CF_ACCOUNT_ID}/images/v1?per_page=1`, { headers: { "Authorization": `Bearer ${env.CF_IMAGES_API_TOKEN}` } });
  let data = null;
  try { data = await r.json(); } catch {}
  return json({ ok: r.ok && data?.success === true, status: r.status, cf_success: data?.success ?? null, cf_errors: data?.errors ?? null, account_hash: env.CF_IMAGES_ACCOUNT_HASH, sample_count: Array.isArray(data?.result?.images) ? data.result.images.length : null, hint: r.ok && data?.success ? "Images token is valid — ready for branded variants + uploads." : "Images token is invalid or lacks Cloudflare-Images:Edit scope." }, r.ok && data?.success ? 200 : 502);
}

async function streamTokenTest(env) {
  if (!env.CF_ACCOUNT_ID) return json({ ok: false, reason: "CF_ACCOUNT_ID not set" }, 503);
  if (!env.CF_STREAM_API_TOKEN) return json({ ok: false, reason: "CF_STREAM_API_TOKEN not set" }, 503);
  const r = await fetch(`https://api.cloudflare.com/client/v4/accounts/${env.CF_ACCOUNT_ID}/stream?limit=1`, { headers: { "Authorization": `Bearer ${env.CF_STREAM_API_TOKEN}` } });
  let data = null;
  try { data = await r.json(); } catch {}
  return json({ ok: r.ok && data?.success === true, status: r.status, cf_success: data?.success ?? null, cf_errors: data?.errors ?? null, sample_count: Array.isArray(data?.result) ? data.result.length : null, hint: r.ok && data?.success ? "Stream token is valid — next render will upload to Stream and use the iframe player." : "Stream token is invalid or lacks Stream:Edit. Renders will fall back to native HTML5 player from R2." }, r.ok && data?.success ? 200 : 502);
}
...
async function listHeygenTemplates(env) {
  const entries = await Promise.all(
    Object.entries(VIDEO_TYPE_TEMPLATE_VAR).map(async ([videoType, varName]) => {
      ...
      const row = { video_type: videoType, env_var: varName, template_id: templateId, edit_url: templateId ? `https://app.heygen.com/templates/${templateId}` : "https://app.heygen.com/templates" };
      ...
      return { ...row, status: "ok", name: details.name || details.title || null, thumbnail_url: details.thumbnail_image_url || details.thumbnail_url || details.cover_image_url || details.cover_url || details.image_url || details.preview_image_url || null, video_url: details.video_url || details.preview_video_url || null, variables: Object.keys(details.variables || {}), _raw_keys: Object.keys(details).slice(0, 20) };
    })
  );
...
async function healthDeep(env) {
  ...
  const out = {
    service: "ai-video-system", env: env.ENVIRONMENT, build: "v4-2026-06-02-await-pipeline",
    heygen_mode: isProduction ? "live (paid credits)" : "test (free)",
    cron_enabled: false, delivery_path: "heygen_webhook_only",
    bindings: { VIDEO_BUCKET: !!env.VIDEO_BUCKET, PREVIEW_BUCKET: !!env.PREVIEW_BUCKET, VIDEO_KV: !!env.VIDEO_KV, RENDER_QUEUE: !!env.RENDER_QUEUE },
    upstreams: { heygen: !!env.HEYGEN_API_KEY, fcpxml: !!env.FCPXML_MCP_URL, cf_stream: !!env.CF_STREAM_API_TOKEN, cf_images: !!env.CF_IMAGES_API_TOKEN, ghl: !!(env.GHL_V2_TOKEN || env.GHL_API_KEY), supabase: !!(env.SUPABASE_URL && env.SUPABASE_KEY), anthropic: !!env.ANTHROPIC_API_KEY, openai: !!env.OPENAI_API_KEY },
    counters: {},
    kill_switch: await killSwitchState(env),
  };

  if (env.SUPABASE_URL && env.SUPABASE_KEY) {
    const r = await fetch(`${env.SUPABASE_URL}/rest/v1/video_jobs?select=status&limit=1000`, { headers: { ...sbHeaders(env), "Prefer": "count=exact" } });
    if (r.ok) {
      const rows = await r.json();
      const tally = {};
...
async function ghlWebhooksList(env) {
  ...
  const r = await fetch(`${GHL_BASE}/webhooks?locationId=${encodeURIComponent(locId)}`, { headers: ghlAuthHeaders(env) });
  if (!r.ok) {
...
async function ghlWebhooksRegister(env) {
  ...
  const listR = await fetch(`${GHL_BASE}/webhooks?locationId=${encodeURIComponent(locId)}`, { headers: ghlAuthHeaders(env) });
  if (listR.ok) {
    ...
    if (existing) return json({ ok: true, already_registered: true, webhook: existing });
  }

  const r = await fetch(`${GHL_BASE}/webhooks`, {
    method: "POST",
    headers: ghlAuthHeaders(env),
    body: JSON.stringify({ locationId: locId, name, url: webhookUrl, events: ["ContactTagUpdate"] }),
  });
